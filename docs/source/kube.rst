Scaling [iotsolution-3f10] With Kubernetes
===========================
...
Based on your TML solution [iotsolution-3f10] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`iotsolution-3f10.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-iotsolution-3f10.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingressiotsolution-3f10.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f iotsolution-3f10.yml

iotsolution-3f10.yml
------------------------

.. important::
   Copy and Paste this YAML file: iotsolution-3f10.yml - and save it locally.

   These YML files can also be found on GitHub at: https://github.com/smaurice101/raspberrypitss/tree/main/tml-airflow/dags/tml-solutions/iotsolution-3f10/ymls/iotsolution-3f10
...
.. code-block:: YAML
   ...
   ################# iotsolution-3f10.yml
     ...
     kind: Deployment
     metadata:
       name: iotsolution-3f10
     spec:
       selector:
         matchLabels:
           app: iotsolution-3f10
       ...
       template:
         ...
         spec:
           containers:
           - name: iotsolution-3f10
             image: maadsdocker/iotsolution-3f10-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: PROJECTNAME
               value: 'iotsolution-3f10'                              
             - name: SOLUTIONNAME
               value: 'iotsolution-3f10'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_dag-iotsolution-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
   ...
   ---
     ...
     metadata:
       ...
       labels:
         app: iotsolution-3f10-visualization-service
     spec:
       type: ClusterIP
       ports:
...
.. code-block::
   ...
   kubectl port-forward deployment/iotsolution-3f10 6060:6060
